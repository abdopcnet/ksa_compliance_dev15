...
ksa_compliance.feedback_dialog = {
  show_feedback_dialog: async function (
    title,
    company = frappe.defaults.get_user_default("Company"),
    is_onboard_feedback = false
  ) {
    const uploaded_files = [];
    const feedback_config = await this.get_feedback_configuration();

    const dialog = this.create_feedback_dialog(
      title,
      is_onboard_feedback,
      feedback_config,
      uploaded_files,
  ...
  create_feedback_dialog: function (
    title,
    is_onboard_feedback,
    config,
    uploaded_files,
    ...
    const fields = [
      ...
      },
      {
        ...
        fieldtype: "Text",
        reqd: 1,
        description: __("Maximum {0} characters", [
      ...
      },
      {
        ...
        click() {
          ksa_compliance.feedback_dialog.create_file_uploader(
            config,
            uploaded_files
          );
        },
      },
      {
        ...
        options: `<div class="alert alert-warning">
                    <strong>${__("Notes")}:</strong> ${__(
          "Uploaded files will be publicly accessible and visible to us from your server. Please do not upload sensitive information."
        )}
                    <br>
                    ${__(
                      "We will use the company phone number and email to contact you regarding your feedback if they are provided."
                    )}
                </div>`,
      },
      {
      ...
      fields.unshift({
        ...
        options: `
                    <div style="margin-bottom: 10px; font-weight: 400;">
                        ${__(
                          "This feedback will be sent to the Application vendor."
                        )}<br>
                        ${__(
                          "How was your experience configuring these settings?"
                        )}
                    </div>
                `,
      });
    ...
    const dialog = new frappe.ui.Dialog({
      ...
      async primary_action(values) {
        try {
          if (
            !ksa_compliance.feedback_dialog.validate_feedback_submission(
              values,
              config
            )
          ) {
          ...
          await ksa_compliance.feedback_dialog.submit_feedback(
            values,
            uploaded_files,
            dialog,
            company
  ...
  validate_feedback_submission: function (values, config) {
    if (values.description.length > config.MAX_DESCRIPTION_LENGTH) {
      frappe.msgprint(
        __("Description must be less than {0} characters", [
          config.MAX_DESCRIPTION_LENGTH,
        ])
      );
      return false;
  ...
  create_file_uploader: function (config, uploaded_files) {
    new frappe.ui.FileUploader({
      ...
      upload_notes: __(
        "Upload up to {0} files (PDF, PNG, JPEG, DOCX), max {1}MB each",
      ...
      on_success(file) {
        ...
        if (file_preview) {
          file_preview.innerHTML = uploaded_files.length
            ? uploaded_files
                .map(
                  (file) => `
                        <div style="margin-top: 5px;">
