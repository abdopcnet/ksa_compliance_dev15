...
def validate_mandatory_crn(doc):
    if (
        ...
        if not crn:
            fthrow(
                msg=ft(
                    'CRN is mandatory when ZATCA branch configuration is enabled for company.'),
...
def validate_duplicate_crn(doc):
    if doc.custom_branch_ids:
        ...
        crn_exists = frappe.db.exists(
            ...
            {'parentfield': 'custom_branch_ids', 'parenttype': 'Branch',
                'value': crn, 'parent': ['!=', doc.name]},
        )
        if crn and crn_exists:
            branch = frappe.get_value(
                'Additional Seller IDs', crn_exists, 'parent')
            fthrow(msg=ft('This CRN is configured for branch: $branch',
                   branch=branch), title=ft('Duplicate CRN Error'))
