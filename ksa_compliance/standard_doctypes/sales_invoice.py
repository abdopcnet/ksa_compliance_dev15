...
def update_sales_invoice_fields(
    ...
    is_phase_1_enabled = ZATCAPhase1BusinessSettings.is_enabled_for_company(self.company)
    is_phase_2_enabled = ZATCABusinessSettings.is_enabled_for_company(self.company)
...
def _update_phase_1_fields(invoice: SalesInvoice | POSInvoice) -> None:
    ...
    frappe.db.set_value(invoice.doctype, invoice.name, update_values, update_modified=False)
...
def update_sales_invoice_from_siaf(doc: SalesInvoiceAdditionalFields, method: str) -> None:
    ...
    try:
        invoice = cast(SalesInvoice | POSInvoice, frappe.get_doc(invoice_doctype, doc.sales_invoice))
    except frappe.DoesNotExistError:
        logger.error(f"SIAF {doc.name} references missing {invoice_doctype} {doc.sales_invoice}")
        return
