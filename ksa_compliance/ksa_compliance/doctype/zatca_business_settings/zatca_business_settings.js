frappe.ui.form.on("ZATCA Business Settings", {
  ...
  refresh: async function (frm) {
    ...
    frm.add_custom_button(__("Submit Feedback"), () => {
      ksa_compliance.feedback_dialog.show_feedback_dialog(
        __("Submit Feedback"),
        frm.doc.company
      );
    });
  ...
  create_csr: function (frm) {
    frappe.prompt(__("OTP"), async ({ value }) => {
      await frappe.call({
        ...
        freeze_message: __("Please wait..."),
        method:
          "ksa_compliance.ksa_compliance.doctype.zatca_business_settings.zatca_business_settings.onboard",
        args: {
  ...
  perform_compliance_checks: function (frm) {
    if (!frm.doc.compliance_request_id) {
      frappe.throw(
        __("Please Onboard first to generate a compliance request ID")
      );
      return;
    }

    let simplified =
      frm.doc.type_of_business_transactions ===
        "Let the system decide (both)" ||
  ...
  get_production_csid: function (frm) {
    ...
    frappe.prompt(__("OTP"), async ({ value }) => {
      const result = await frappe.call({
        ...
        freeze_message: "Please wait...",
        method:
          "ksa_compliance.ksa_compliance.doctype.zatca_business_settings.zatca_business_settings.get_production_csid",
        args: {
          business_settings_id: frm.doc.name,
          otp: value,
        },
...
function add_revoke_button(frm) {
  frm
    .add_custom_button("Revoke", async () => {
      frappe.confirm(
        __(
          "Are you sure you want to revoke this Business Settings and CSID? This cannot be undone."
        ),
        () => {
          frappe.call({
            method:
              "ksa_compliance.ksa_compliance.doctype.zatca_business_settings.zatca_business_settings.revoke_business_settings",
            args: {
...
function add_create_business_settings_button(frm) {
  frm.add_custom_button("Create New Business Settings", () => {
    frappe.model.open_mapped_doc({
      method:
        "ksa_compliance.ksa_compliance.doctype.zatca_business_settings.zatca_business_settings.create_business_settings",
      frm: cur_frm,
    });
  });
}
