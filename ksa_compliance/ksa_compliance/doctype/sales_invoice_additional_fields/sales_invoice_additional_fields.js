...
frappe.ui.form.on('Sales Invoice Additional Fields', {
	refresh: function (frm) {
		if (
			frm.doc.integration_status === 'Rejected' &&
			!frm.doc.precomputed_invoice &&
			frm.doc.is_latest
		) {
	...
	download_xml: function (frm) {
		window.open(
			'/api/method/ksa_compliance.ksa_compliance.doctype.sales_invoice_additional_fields.sales_invoice_additional_fields.download_xml?id=' +
				frm.doc.name,
		);
	},
	download_zatca_pdf: async function (frm) {
		...
		frappe.prompt(fields, (values) => {
			...
			window.location.assign(
				'/api/method/ksa_compliance.ksa_compliance.doctype.sales_invoice_additional_fields.sales_invoice_additional_fields.download_zatca_pdf?id=' +
					frm.doc.name +
					'&print_format=' +
					print_format +
					'&lang=' +
